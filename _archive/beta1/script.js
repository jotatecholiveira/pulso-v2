...
function renderHeaderUser() {
  ...
  if (badge && currentUser) badge.textContent = '👤 ' + (currentUser.email || 'usuário');
}
...
function renderDashGreeting() {
  ...
  if (userEl) userEl.textContent = currentUser ? (currentUser.email || '').split('@')[0] : 'Usuário';
...
function renderizarLimites() {
  ...
  const categories = {};
  transactions.forEach(t => {
    if (t.type === 'saida' && t.category) {
      categories[t.category] = (categories[t.category] || 0) + Math.abs(parseFloat(t.value) || 0);
    }
  });
